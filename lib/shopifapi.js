// Generated by CoffeeScript 1.7.1
(function() {
  var Log, Queue, Shopifapi, Sleep, async, req;

  Queue = require('burst-queue');

  async = require('async');

  req = require('request');

  Log = require('./log');

  Sleep = require('sleep');

  Shopifapi = (function() {
    var _auth, _buildOptions, _log, _queue, _url;

    _auth = {
      username: '',
      password: '',
      base64: ''
    };

    _url = '';

    _log = null;

    _queue = null;

    _buildOptions = function(url, method, data) {
      var options;
      options = {
        url: url,
        method: method,
        json: true,
        headers: {
          "Authorization": _auth.base64
        }
      };
      if (data != null) {
        options.body = JSON.stringify(data);
      }
      return options;
    };

    function Shopifapi(options) {
      var verbose;
      _auth = options.auth;
      _auth.base64 = "Basic " + ((new Buffer([_auth.username, _auth.password].join(':'))).toString('base64'));
      _url = options.url;
      verbose = options.verbose != null ? options.verbose : false;
      _log = new Log(verbose, options.logger);
      _queue = new Queue(5 * 60 * 1000, 250);
    }

    Shopifapi.prototype.getBaseObj = function(obj, args, callback) {
      var argswa, argswq, countComplete, countUrl, getData;
      argswq = '';
      argswa = '';
      if (args) {
        argswq = '?' + args;
        argswa = '&' + args;
      }
      countUrl = "" + _url + "/admin/" + obj + "/count.json" + argswq;
      countComplete = function(err, response, result) {
        var currPage, objects, pages, q, qtask, _i, _results;
        pages = Math.ceil(result.count / 250);
        objects = [];
        qtask = function(task, callback) {
          var fetch, options;
          fetch = "" + _url + "/admin/" + obj + ".json?limit=250&page=" + task.page + argswa;
          options = _buildOptions(fetch, 'GET');
          return _queue.add(function() {
            var request;
            _log.may(fetch);
            return request = req(options, function(err, response, result) {
              if (err != null) {
                _log.may(err);
              }
              if (response.statusCode === 500) {
                _log.may("ERROR 500. Retrying...");
                return this.retry(0);
              } else {
                objects = objects.concat(result[obj]);
                if (callback != null) {
                  return callback(objects);
                }
              }
            });
          });
        };
        q = async.queue(qtask, 5);
        q.drain = function() {
          if (callback != null) {
            return callback(objects);
          }
        };
        _results = [];
        for (currPage = _i = 1; 1 <= pages ? _i <= pages : _i >= pages; currPage = 1 <= pages ? ++_i : --_i) {
          _results.push(q.push({
            page: currPage
          }));
        }
        return _results;
      };
      getData = (function(_this) {
        return function() {
          var options;
          _log.may(countUrl);
          options = {
            url: countUrl,
            method: 'GET',
            json: true,
            headers: {
              "Authorization": _auth.base64
            }
          };
          _log.may(options);
          return req(options, countComplete);
        };
      })(this);
      return _queue.add(getData);
    };

    Shopifapi.prototype.getBaseObjById = function(obj, id, callback) {
      var fetch, options;
      fetch = "" + _url + "/admin/" + obj + "/" + id + ".json";
      options = _buildOptions(fetch, 'GET');
      return this.makeRequest(null, fetch, options, callback);
    };

    Shopifapi.prototype.getSubObj = function(obj, subobj, args, callback) {
      var fetch, options;
      fetch = "" + _url + "/admin/" + obj + "/" + args + "/" + subobj + ".json";
      options = _buildOptions(fetch, 'GET');
      return this.makeRequest(subobj, fetch, options, callback);
    };

    Shopifapi.prototype.makeRequest = function(obj, url, options, callback) {
      _queue.add(function() {
        _log.may(url);
        return req(options, function(err, response, result) {
          var data;
          if (response.statusCode === 500) {
            _log.may("ERROR 500. Retrying...");
            this.retry(0);
          }
          if (err != null) {
            _log.may(err);
          }
          if (result != null) {
            _log.may(result);
          }
          data = null;
          if (result != null) {
            if (obj in result) {
              data = result[obj];
            } else {
              data = result;
            }
          }
          if (callback != null) {
            return callback(data);
          }
        });
      });
      return Sleep.usleep(500000);
    };

    Shopifapi.prototype.put = function(obj, id, data, callback) {
      var options, put;
      put = "" + _url + "/admin/" + obj + "/" + id + ".json";
      options = _buildOptions(put, 'PUT', data);
      return this.makeRequest(obj, put, options, callback);
    };

    Shopifapi.prototype.post = function(obj, data, callback) {
      var options, post;
      post = "" + _url + "/admin/" + obj + ".json";
      options = _buildOptions(post, 'POST', data);
      return this.makeRequest(obj, post, options, callback);
    };

    Shopifapi.prototype["delete"] = function(obj, id, callback) {
      var del, options;
      del = "" + _url + "/admin/" + obj + "/" + id + ".json";
      options = _buildOptions(del, 'DELETE');
      return this.makeRequest(obj, del, options, callback);
    };

    Shopifapi.prototype.queue = function() {
      return _queue.enqueued();
    };

    return Shopifapi;

  })();

  module.exports = Shopifapi;

}).call(this);
